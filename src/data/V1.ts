import { JSBI, Pair, Percent, Route, Token, TokenAmount, Trade, TradeType, WETH } from '@AIHISwap/sdk'
...
export function useUserHasLiquidityInAllTokens(): boolean | undefined {
  ...
  const fakeLiquidityTokens = useMemo(
    () =>
      chainId ? Object.keys(exchanges).map(address => new Token(chainId, address, 18, 'UNI-V1', 'AIHISwap V1')) : [],
    [chainId, exchanges]
  )
